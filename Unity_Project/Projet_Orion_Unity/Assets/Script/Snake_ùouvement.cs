...
public class Snake_ùouvement : MonoBehaviour
{
	...
	void Start () {
		...
		Transform newTail = Instantiate(Tail, Bodys[Bodys.Count - 1].position, Bodys[Bodys.Count - 1].rotation).transform;
	...
	public void AddBody()
	{
		...
		{
			Transform newBody = Instantiate(Body, Bodys[Bodys.Count - 2].position, Bodys[Bodys.Count - 2].rotation).transform;
